...
let getWeatherDetails = (y) => {
    var api_key = 'eb8ea9f133d75bd85238f7fc85a99b09'
    var weatherUrl = `http://api.openweathermap.org/data/2.5/weather?q=${y}&appid=${api_key}`

    var w = fetch(weatherUrl)

    w.then((resultw) => {
        return resultw.json()
    }).then((dataw) => {
        var buttonReplace = document.querySelector(`#${y}`)
        buttonReplace.innerHTML = `Temperature is = ${(dataw.main.temp -273.15).toFixed(1)} &#8451`
    }).catch((err) => {
        console.log(err)
    })
}